...
namespace _05._BinarySearchTree
{
	public class Tree
	{
		...
		* 트리 : 부모로 다시 돌아올 수 없다. 계층구조
		* 그래프 : 부모로 다시 돌아올 수 있다. 맵, 연결성
		********************************************************************************/
		...
		// 깊이(Depth)  : 루트 노드부터의 길이(뿌리 0부터 시작)
		// 차수(Degree) : 자식노드의 갯수
		//
		//             뿌리                  - 깊이 0
		//        ┌─────┼─────┐
		...
		{
			public T item;
			public TreeNode<T> parent; // 부모노드의 참조, 내용 == null 루트(뿌리) 노드 
			public List<TreeNode<T>> children; // 자식들을 저장, count == 0 리프(잎) 노드
		}
		...
		// 전위순회(Pre-Order)  : 노드, 왼쪽, 오른쪽 // 깊이우선탐색 방식
