namespace _02._LinkedList
{
	internal class Program
	{
		...
		static void Main(string[] args)
		{
			...
			LinkedListNode<string> beforeNode0 = linkedList.AddBefore(node0, "0번 앞에 데이터 Add");
			LinkedListNode<string> afterNode0 = linkedList.AddAfter(node0, "0번 뒤에 데이터 Add");


			// 삭제
			bool success = linkedList.Remove("1번 데이터"); // 있으면 지우고 true 반환
			bool fail = linkedList.Remove("1번 데이터"); // 없었으면 못지우고 false 반환
			linkedList.Remove(node3); // 이 방법 쓰자 O(1) 위에는 O(n)
			...
			LinkedListNode<string> prevNode = node0.Previous; // node0 기준 앞(이전거)
			LinkedListNode<string> nextNode = node0.Next; // node0 기준 뒤(다음거)
